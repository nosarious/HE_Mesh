	...
	public int[] getTriangles(final boolean optimize) {
		if (triangles == null) {
			if (numberOfShellPoints < 3) {
				...
				return WB_PolygonTriangulatorEarcut.triangulateQuad(points.get(0), points.get(1), points.get(2),
						points.get(3));
			} else {
				final WB_Triangulation2D triangulation = WB_PolygonTriangulatorEarcut
						.triangulatePolygon2D(this.toPolygon2D());
