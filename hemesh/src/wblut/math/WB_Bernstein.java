...
public class WB_Bernstein {
     ...
     */
    public static double[] getBernsteinCoefficientsOfOrderN(final double u,
	    final int order) {
     ...
     *         is the coefficient
     */
    public static double[][] getBernsteinCoefficientsOfOrderNForRange(
	    final int steps, final int order) {
	...
	for (int i = 0; i < clampedsteps; i++) {
	    B[i][0] = 1.0;
	    u = (i == (clampedsteps - 1)) ? 1.0 : i
		    * (1.0 / (clampedsteps - 1.0));
	    u1 = 1.0 - u;
	    for (int j = 1; j <= order; j++) {
		saved = 0.0;
		for (int k = 0; k < j; k++) {
