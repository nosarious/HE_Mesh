...
public class WB_GeomGrid {
	 ...
	 * @param D
	 */
	public WB_GeomGrid(final double minx, final double miny, final double minz, final double maxx, final double maxy,
			final double maxz, final int W, final int H, final int D) {
		this.W = W;
		this.H = H;
		this.D = D;
		WH = W * H;
	 ...
	 * @param r
	 */
	public void addPoint(final WB_Coord p, final double r) {
		final ArrayList<WB_GeomGridCell> fatcells = getCellsInNeighborhood(p, r, true);
	...
	public void addSegment(final WB_Segment S, final double r) {
		final ArrayList<WB_GeomGridCell> fatcells = getCellsInNeighborhood(S, r, true);
		for (final WB_GeomGridCell fatcell : fatcells) {
			final int id = fatcell.getIndex();
			final WB_GeomGridCell cell = cells.get(id);
			if (cell == null) {
	 ...
	 * @return
	 */
	public WB_Point index(final WB_Coord p) {
	 ...
	 * @return
	 */
	public WB_Point safeIndex(final WB_Coord p) {
		final Index id = safeijk(p);
		if (id == null) {
			return null;
		}
	 ...
	 * @return
	 */
	public ArrayList<WB_Point> getPointsInSameCell(final WB_Coord p) {
		final Index id = safeijk(p);
		if (id == null) {
			return new ArrayList<WB_Point>();
		}
	 ...
	 * @return
	 */
	public ArrayList<WB_GeomGridCell> getCellsInNeighborhood(final WB_Coord p, final double r, final boolean all) {
				...
				for (int dk = -rk; dk <= rk; dk++) {
					...
					if (neighbor > -1) {
						...
						if (cell != null) {
							...
							cell = getNewCellForIndex(id.i + di, id.j + dj, id.k + dk);
							if (WB_GeometryOp.getSqDistance3D(p, cell.getAABB()) <= r2) {
								result.add(cell);
							}
						}
	 ...
	 * @return
	 */
	public ArrayList<WB_GeomGridCell> getCellsInNeighborhood(final WB_Segment S, final double r, final boolean all) {
		final ArrayList<WB_GeomGridCell> result = new ArrayList<WB_GeomGridCell>();
			...
			for (int dj = js; dj <= je; dj++) {
				for (int dk = ks; dk <= ke; dk++) {
					...
					if (neighbor > -1) {
						cell = cells.get(neighbor);
						if (cell != null) {
							if (WB_GeometryOp.getSqDistance3D(cell.getAABB().getCenter(), S) <= r2) {
								result.add(cell);
							}
						} else if (all) {
	...
	public ArrayList<Index> indicesTraversed(final WB_Segment segment) {
		...
		final WB_Coord dir = segment.getDirection();
		double x, y, z;
		Index current;
		Index prev;
		...
		final double idx = (WB_Epsilon.isZero(dir.xd())) ? Double.POSITIVE_INFINITY : 1.0 / dir.xd();
		final double idy = (WB_Epsilon.isZero(dir.yd())) ? Double.POSITIVE_INFINITY : 1.0 / dir.yd();
		final double idz = (WB_Epsilon.isZero(dir.zd())) ? Double.POSITIVE_INFINITY : 1.0 / dir.zd();
		...
		do {
			...
			if (current.inside) {
				final Index newindex = new Index(current.i, current.j, current.k, current.inside);
				if (!newindex.equals(prev)) {
					indicesTraversed.add(newindex);
				}
			}
		} while (!current.equals(end) && ((signx * current.i) <= (signx * end.i))
				&& ((signy * current.j) <= (signy * end.j)) && ((signz * current.k) <= (signz * end.k))
				&& (!current.equals(prev)));
		return indicesTraversed;
	}
	...
	private WB_GeomGridCell getNewCellForIndex(final Index id) {
		return new WB_GeomGridCell(index(id),
				new WB_Point((id.i * dx) + min.xd(), (id.j * dy) + min.yd(), (id.k * dz) + min.zd()),
	...
	private WB_GeomGridCell getNewCellForIndex(final int i, final int j, final int k) {
		return new WB_GeomGridCell(index(i, j, k),
				new WB_Point((i * dx) + min.xd(), (j * dy) + min.yd(), (k * dz) + min.zd()),
				new WB_Point((i * dx) + min.xd() + dx, (j * dy) + min.yd() + dy, (k * dz) + min.zd() + dz));
	}
	...
	public class WB_GeomGridCell {
		 ...
		 * @param max
		 */
		public WB_GeomGridCell(final int index, final WB_Coord min, final WB_Coord max) {
			this.index = index;
