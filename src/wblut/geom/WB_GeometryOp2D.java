...
public class WB_GeometryOp2D extends WB_GeometryOpGLU {
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Line L, final WB_Circle C) {
		...
		final double c = WB_GeometryOp.getSqLength2D(C.getCenter()) + WB_Vector.getSqLength2D(L.getOrigin())
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Ray R, final WB_Circle C) {
		...
		final double c = WB_GeometryOp.getSqLength2D(C.getCenter()) + WB_Vector.getSqLength2D(R.getOrigin())
	...
	public static final WB_Point getClosestPoint2D(final WB_Coord p, final WB_PolyLine PL) {
		...
		for (int s = 0; s < PL.getNumberSegments(); s++) {
			...
			d2 = WB_GeometryOp.getSqDistance2D(p, test);
			if (d2 < d2min) {
				d2min = d2;
				q = test;
			}
	 ...
	 * @return
	 */
	public static final boolean coincident2D(final WB_Coord a, final WB_Coord b) {
	...
	public static final double getAngleBetween2D(final WB_Coord c, final WB_Coord p, final WB_Coord q) {
		final WB_Vector v0 = gf.createVectorFromTo2D(c, p);
		final WB_Vector v1 = gf.createVectorFromTo2D(c, q);
		v0.normalizeSelf();
		v1.normalizeSelf();
		double d = v0.dot2D(v1);
		if (d < -1.0) {
	 ...
	 * @return
	 */
	public static final boolean isZero2D(final double ux, final double uy, final double uz) {
	...
	public static final WB_Circle fitCircleToPoints2D(final Collection<? extends WB_Coord> points) {
		...
		EigenDecomposition<DenseMatrix64F> eigen = DecompositionFactory.eig(4, true, true);
		eigen.decompose(A);
		int numEigenValues = eigen.getNumberOfEigenvalues();
		int index = 0;
		for (int i = 1; i < numEigenValues; i++) {
			if (eigen.getEigenvalue(i).real < eigen.getEigenvalue(index).real) {
				index = i;
			}

		}
		DenseMatrix64F eigenVector0 = eigen.getEigenVector(index);
