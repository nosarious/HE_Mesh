...
public class WB_RBSpline extends WB_BSpline {
     ...
     * @param knot 
     */
    public WB_RBSpline(final WB_PointHomogeneous[] controlPoints,
	...
	final WB_PointHomogeneous CH = new WB_PointHomogeneous();
	final int p = knot.p();
	for (int i = 0; i <= p; i++) {
	...
	for (int j = 1; j <= r; j++) {
		...
		RW[i] = WB_PointHomogeneous
			.interpolate(RW[i], RW[i + 1], alpha);
	    }
	    Q[L] = RW[0];
	    Q[(k + r) - j - s] = RW[p - j - s];
	}
	...
	for (int j = r; j >= 0; j--) {
		...
		if (WB_Epsilon.isZero(alpha)) {
		    ...
		    Q[ind - 1] = WB_PointHomogeneous.interpolate(Q[ind],
			    Q[ind - 1], alpha);
		}
	    }
	    Ubar.setValue(k, X[j]);
	...
	final int s = knot.s();
	final WB_PointHomogeneous[] Q = new WB_PointHomogeneous[n
		+ (t * (s + 1)) + 1];
	...
	while (b < m) {
		...
		for (j = 1; j <= r; j++) {
		    ...
		    final int sj = mul + j;
		    for (k = p; k >= sj; k--) {
			bpts[k] = WB_PointHomogeneous.interpolate(bpts[k - 1],
				bpts[k], alfs[k - sj]);
		    }
		...
		for (int tr = 1; tr < oldr; tr++) {
			...
			if (i < cind) {
			    final double alf = (ub - Uh.value(i))
				    / (ua - Uh.value(i));
			    Q[i] = WB_PointHomogeneous.interpolate(Q[i - 1],
				    Q[i], alf);
			}
			if (j >= lbz) {
				...
				final double gam = (ub - Uh.value(j - tr))
					/ den;
				ebpts[kj] = WB_PointHomogeneous.interpolate(
