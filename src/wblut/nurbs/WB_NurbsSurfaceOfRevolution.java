...
public class WB_NurbsSurfaceOfRevolution {
	 ...
	 * @return
	 */
	public static WB_RBSplineSurface getSurfaceOfRevolution(final WB_BSpline C, final WB_Coord p,
		...
		for (j = 0; j <= C.n(); j++) {
			...
			for (i = 1; i <= narcs; i++) {
				...
				final WB_Coord p1 = (is.dimension == 0) ? (WB_Point) is.object
	 ...
	 * @return
	 */
	public static WB_RBSplineSurface getSurfaceOfRevolution(final WB_RBSpline C, final WB_Coord p,
			final WB_Coord axis, double theta) {
		final WB_Vector v = new WB_Vector(axis);
		if (theta < 0) {
			theta *= -1;
