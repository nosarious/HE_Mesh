...
public class WB_BSpline implements WB_Curve {
	 ...
	 * @param knot
	 */
	public WB_BSpline(final WB_Point[] controlPoints, final WB_NurbsKnot knot) {
	...
	public WB_BSpline refineKnot(final double[] X) {
		...
		for (int i = 0; i < r; i++) {
			if (X[i] > X[i + 1]) {
				throw new IllegalArgumentException("Provided values are not non-decreasing.");
			}
		}
		int id = 0;
	 ...
	 * @return
	 */
	public WB_Point[][] curveDerivCPoints(final int d, final int r1, final int r2) {
		...
		for (int k = 1; k <= d; k++) {
			final int tmp = (p - k) + 1;
			for (int i = 0; i <= (r - k); i++) {
				PK[k][i] = PK[k - 1][i + 1].sub(PK[k - 1][i])
