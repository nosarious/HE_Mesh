...
public class HEMC_WeairePhelan extends HEMC_MultiCreator {
	...
	private HE_Mesh[] singleCell(final WB_Vector offset) {
		...
		cells[7].move(0, 1, 0);
		for (int i = 0; i < 8; i++) {
			cells[i].scale(0.5 * scU, 0.5 * scV, 0.5 * scW, new WB_Point(0, 0, 0));
			cells[i].move(offset);
	...
	public HE_MeshCollection create() {
		...
		for (int i = 0; i < (U + 1); i++) {
			for (int j = 0; j < (V + 1); j++) {
				for (int k = 0; k < (W + 1); k++) {
					final WB_Vector offset = new WB_Vector(origin.xd() + ((i - 0.5) * scU),
							origin.yd() + ((j - 0.5) * scV), origin.zd() + ((k - 0.5) * scW));
