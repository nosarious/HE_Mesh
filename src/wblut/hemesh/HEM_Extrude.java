	...
	public HE_Mesh apply(final HE_Mesh mesh) {
		...
		for (int i = 0; i < nf; i++) {
			...
			do {
				...
				_halfedgeEWs.put(he.key(),
						(he.getHalfedgeDihedralAngle() < thresholdAngle) ? hardEdgeChamfer : chamfer);
				he = he.getNextInFace();
				...
				for (int i = 0; i < faces.size(); i++) {
					...
					if (!failedFaces.contains(f)) {
						...
						do {
							...
							he.getVertex().getPoint().addMulSelf(d.evaluate(v.xd(), v.yd(), v.zd()), n);
							he = he.getNextInFace();
						} while (he != f.getHalfedge());
					}
				}
		...
		if (!flat) {
			if (heights != null) {
				...
				for (int i = 0; i < selFaces.size(); i++) {
					...
					do {
						final HE_Vertex v = he.getVertex();
						v.getPoint().addMulSelf(d.evaluate(v.xd(), v.yd(), v.zd()), n);
						he = he.getNextInFace();
					} while (he != f.getHalfedge());
				}
			}
		}
		selection.parent.cleanUVW();
		tracker.setStatus(this, "Exiting HEM_Extrude.", -1);
		return selection.parent;
	}
		...
		if (relative == true) {
			...
			if ((chamfer > 0) && (f.getFaceType() == WB_Classification.CONVEX)) {
				...
				for (int i = 0, j = n - 1; i < n; j = i, i++) {
					...
					final WB_Point v3 = faceVertices.get((i + 1) % n).getPoint();
					final WB_Segment S1 = new WB_Segment(v1.addMul(d[j], n1), v2.addMul(d[j], n1));
					final WB_Segment S2 = new WB_Segment(v2.addMul(d[i], n2), v3.addMul(d[i], n2));
