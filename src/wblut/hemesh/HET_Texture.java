...
public class HET_Texture {
	...
	private static WB_Coord cartesianToSpherical(final WB_Coord p) {

		double azimuth = Math.atan2(p.yd(), p.xd()) + Math.PI;
		double polar = Math.asin(Math.min(1.0, Math.max(-1, p.zd())));
		return new WB_Point(0, azimuth, polar);
	}

	public static void setUVWFromNormal(final HE_Mesh mesh, final boolean invertZ) {
		clearUVW(mesh);
		HE_VertexIterator vItr = mesh.vItr();
		HE_Vertex v;
		WB_Coord s;
		while (vItr.hasNext()) {
			v = vItr.next();
			s = cartesianToSpherical(v.getVertexNormal());
			v.setUVW(s.yd() * 0.5 / Math.PI, (invertZ ? -1 : 1) * (0.5 + s.zd()) / Math.PI, 0);
		}
	}
	...
	public static void setVertexColorFromVertexNormal(final HE_Mesh mesh) {
		...
		while (vitr.hasNext()) {
			v = vitr.next();
			n = v.getVertexNormal();
			color = color((int) (128 * (n.xd() + 1)), (int) (128 * (n.yd() + 1)), (int) (128 * (n.zd() + 1)));
			v.setColor(color);
		}
	}
	 ...
	 * @param palette
	 */
	public static void setVertexColorFromPalette(final HE_Mesh mesh, final Palette palette) {
		final HE_VertexIterator vitr = mesh.vItr();
		HE_Vertex v;
	 ...
	 * @param palette
	 */
	public static void setRandomVertexColorFromPalette(final HE_Mesh mesh, final Palette palette) {
		final HE_VertexIterator vitr = mesh.vItr();
		HE_Vertex v;
		final int size = palette.numSwatches();
		int color;
	 ...
	 * @param seed
	 */
	public static void setRandomVertexColorFromPalette(final HE_Mesh mesh, final Palette palette, final long seed) {
		final HE_VertexIterator vitr = mesh.vItr();
		HE_Vertex v;
		final int size = palette.numSwatches();
		int color;
	 ...
	 * @param palette
	 */
	public static void setRandomFaceColorFromPalette(final HE_Mesh mesh, final Palette palette) {
		final HE_FaceIterator fitr = mesh.fItr();
		HE_Face f;
		final int size = palette.numSwatches();
		int color;
	 ...
	 * @param seed
	 */
	public static void setRandomFaceColorFromPalette(final HE_Mesh mesh, final Palette palette, final long seed) {
		final HE_FaceIterator fitr = mesh.fItr();
		HE_Face f;
		final int size = palette.numSwatches();
		int color;
	 ...
	 * @param palette
	 */
	public static void setFaceColorFromPalette(final HE_Mesh mesh, final Palette palette) {
		final HE_FaceIterator fitr = mesh.fItr();
		HE_Face f;
		final int size = palette.numSwatches();
		int color;
	...
	// 041497 - created
	// ==============================================================================
	static public int[] rainbow = new int[] { color(0, 0, 0), color(45, 0, 36), color(56, 0, 46), color(60, 0, 49),
