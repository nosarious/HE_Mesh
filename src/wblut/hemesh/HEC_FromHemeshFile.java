...
public class HEC_FromHemeshFile extends HEC_Creator {
	...
	protected HE_Mesh createBase() {
		...
		for (int i = 0; i < numVertices; i++) {
			...
			v._labels=Long.parseLong(subresult[5]);
			...
			if(hasuvw==1){
				v.setUVW(Double.parseDouble(subresult[7]), Double.parseDouble(subresult[8]), Double.parseDouble(subresult[9]));
			}
		...
		for (int i = 0; i < numHalfedges; i++) {
			...
			he._labels=Long.parseLong(subresult[5]);
			hasuvw= Integer.parseInt(subresult[6]);
			if (vid > -1) {
			...
			if(hasuvw==1){
				he.setUVW(Double.parseDouble(subresult[7]), Double.parseDouble(subresult[8]), Double.parseDouble(subresult[9]));
			}
			id++;
		}
		...
		for (int i = 0; i < numFaces; i++) {
			...
			f._labels=Long.parseLong(subresult[3]);
			id++;
		}
