...
public class HEC_SeaShell extends HEC_Creator {
     ...
     * @return 
     */
    public HEC_SeaShell setRange(final double sm, final double sM,
	    final double thetam, final double thetaM) {
	...
	for (int j = 0; j < (thetasteps + 1); j++) {
		...
		x = scale
			* D
			* ((A * sb * Math.cos(theta)) + (hst * ((csp * cto) - (sm
				* ssp * sto)))) * etca;
		y = scale
			* ((A * sb * Math.sin(theta)) + (hst * ((csp * sto) + (sm
				* ssp * cto)))) * etca;
		z = scale
			* ((-A * Math.cos(beta)) + (hst * ssp * Math.cos(mu
				+ (muf * theta)))) * etca;
	...
	for (int j = 0; j < thetasteps; j++) {
		...
		faces[j + (i * thetasteps)][3] = ((j + 1) % (thetasteps + 1))
			+ (i * (thetasteps + 1));
	    }
	}
	final HEC_FromFacelist fl = new HEC_FromFacelist();
	fl.setVertices(vertices).setFaces(faces).setCheckNormals(true)
		.setDuplicate(false);
	...
	if (((W1 + (W1f * theta)) == 0) || ((W2 + (W2f * theta)) == 0)
		|| (N == 0)) {
	    return 0;
	}
	...
	return (1.0 / Math.sqrt((tmp1 * tmp1) + (tmp2 * tmp2)))
		+ k(s, theta, lt);
    }
}
