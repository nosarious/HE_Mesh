...
import wblut.geom.WB_PolygonTriangulatorEarcut;
...
public class HE_Face extends HE_MeshElement implements Comparable<HE_Face> {
	...
	public int[] getTriangles(final boolean optimize) {
		...
		if (fo < 3) {
			...
			return triangles = WB_PolygonTriangulatorEarcut.triangulateQuad(points[0], points[1], points[2], points[3]);
		}
		return triangles = WB_PolygonTriangulatorEarcut.triangulatePolygon2D(this).getTriangles();
	}
